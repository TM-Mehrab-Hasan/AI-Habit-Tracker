...
const HABITS = [
  ...
  { Icon: FaDumbbell, color: "#14b8a6", orbit: "middle", delay: -16, reverse: true },
  // inner orbit
...
export default function OrbitingHabits() {
  ...
  const sunGradient = isDark
    ? "radial-gradient(circle at 30% 25%, rgba(45,212,191,0.4), rgba(45,212,191,0.24) 60%, rgba(13,148,136,0.2))"
    : "radial-gradient(circle at 30% 25%, #ffffff, #ccfbf1 45%, #99f6e4 100%)";

  const sunShadow = isDark
    ? "inset 0 1px 0 rgba(255,255,255,0.35), 0 6px 24px rgba(45,212,191,0.3)"
    : "inset 0 1px 0 rgba(255,255,255,0.9), 0 8px 24px rgba(13,148,136,0.22), 0 2px 6px rgba(13,148,136,0.14)";

  const sunBorder = isDark
    ? "1px solid rgba(255,255,255,0.2)"
    : "1px solid rgba(255,255,255,0.8)";

  const sunIconColor = isDark ? "#99f6e4" : "#0f766e";

  const haloBg = isDark
    ? "radial-gradient(circle, rgba(45,212,191,0.24), transparent 70%)"
  ...
  return (
    <div className="relative mx-auto w-[300px] h-[300px] sm:w-[400px] sm:h-[400px] lg:w-[460px] lg:h-[460px]">
      ...
      {STARS.map((s, i) => (
        <span
          key={i}
          className="absolute rounded-full bg-teal-400/70 dark:bg-teal-200/70"
          style={{
            top: s.top,
            left: s.left,
            width: s.size,
