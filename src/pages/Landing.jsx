    ...
    <div className="min-h-screen">
      <header className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-lg">Health Tracker</span>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-6 pt-10 md:pt-16 pb-16">
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-6 items-center">
          <div className="lg:col-span-8 text-center lg:text-left order-2 lg:order-1">
            <div className="inline-flex items-center gap-1.5 chip mb-5 bg-teal-500/15 text-teal-700 dark:text-teal-300">
              <Sparkles size={12} />
        ...
        <div className="mt-14 md:mt-20 grid md:grid-cols-2 gap-6">
          <div className="card p-6 relative overflow-hidden">
            <div className="text-xs font-semibold uppercase tracking-wider text-teal-600 dark:text-teal-300 mb-2">
              Today
            </div>
            <div className="space-y-3">
              {[
                ...
                <div
                  key={i}
                  className={`flex items-center gap-3 rounded-xl glass p-3 ${h.done ? "ring-1 ring-teal-500/30" : ""
                    }`}
                >
                  <span className="w-9 h-9 rounded-lg bg-teal-500/15 flex items-center justify-center">
                    {h.icon}
                  </span>
                  <div className="flex-1 text-sm font-medium">{h.name}</div>
                  <div className="flex items-center gap-1 text-xs text-muted">
                    <Flame size={12} className="text-teal-600" />
                    {h.streak}
                  </div>
                  <div
          ...
          <div className="card p-6 relative overflow-hidden">
            ...
            <div className="relative">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-teal-600 dark:text-teal-300 mb-2">
                <Sparkles size={12} />
                AI Weekly Report
              </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {features.map((f) => (
            <div key={f.title} className="card p-5">
              <div className="w-10 h-10 rounded-xl bg-teal-500/15 text-teal-600 dark:text-teal-300 flex items-center justify-center mb-3">
                <f.icon size={18} />
              </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16">
        <div className="relative p-10 text-center rounded-2xl overflow-hidden bg-gradient-to-br from-teal-600 to-teal-900 text-white shadow-2xl shadow-teal-500/30">
          <div
            className="absolute inset-0 pointer-events-none opacity-50"
            style={{
              background:
                "radial-gradient(circle at 20% 10%, rgba(255,255,255,0.25), transparent 55%), radial-gradient(circle at 80% 80%, rgba(45,212,191,0.25), transparent 55%)",
            }}
          />
          <div className="relative">
            ...
            <Link
              to="/register"
              className="mt-6 inline-flex items-center gap-2 rounded-xl bg-white text-teal-700 px-5 py-3 text-sm font-semibold hover:bg-teal-50 transition shadow-xl"
            >
              Create my account
              <ArrowRight size={16} />
            </Link>
